...
void multMatPtrVet(MatPtr mat, Vetor v, int m, int n, Vetor res) {
    ...
    // Fazer LOOP UNROLLING com JAM
    if (res) {
            /*
            qual vai ser o passo??
            */
        for(int i = 0; i < m-m%passo; i+= passo) {
            for(int j = 0; j<n; ++j) {
                res[i] += mat[i][j] * v[j];
                res[i + 1] += mat[i + 1][j] * v[j];
                res[i + 2] += mat[i + 2][j] * v[j];
                res[i + 3] += mat[i + 3][j] * v[j];
                res[i + 4] += mat[i + 4][j] * v[j];
                res[i + 5] += mat[i + 5][j] * v[j];
                res[i + 6] += mat[i + 6][j] * v[j];
                res[i + 7] += mat[i + 7][j] * v[j];
            }
        }

        for (int i = m-m%passo; i < m; ++i) {
            for (int j = 0; j < n; i++) {
                res[i] += mat[i][j] * v[j];
            }
        }
